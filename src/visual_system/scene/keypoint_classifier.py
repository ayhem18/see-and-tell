...
def _keypoints_comparison(
    ...
    num_references = len(reference_feats)
...
class KeyPointClassifier:
    ...
    def _build_references(cls, 
                          ...
                          image_extensions: Sequence[str]) -> Dict:
        
        if isinstance(references, Dict):
            try:
                references = {k: [Image.Open(f) for f in v] for k, v in references}
        ...
        if isinstance(references, (str, Path)):
            references = dirf.process_save_path() 
            ...
            for cls_name in os.listdir(references):
                references_map[cls_name] = [Image.open(os.path.join(references, cls_name, im)) for im in os.listdir(os.path.join(references, cls_name))]
            
            return references_map

    def __init__(self, 
                 references: Union[Dict, str, Path],
                 resize: Union[Tuple[int, int], int], 
                 device: str = None, 
                 batch_size: int = 4, 
        ...
        for cls_name, imgs in self.references_location_map:
        ...
        for i in range(0, len(frames), self.batch_size):
            ...
            for cls_name, cls_ref_feats in self.references_feats.items():
                ...
                for j in range(0, len(cls_ref_feats), self.batch_size):
                    ref_batch = cls_ref_feats[j: j + self.batch_size]
            ...
            # time to classify
            batch_preds = [list(cls_score_map.keys())[np.argmax(list(cls_score_map.values()))] 
                           for _, cls_score_map in batch_classification_map.items()]
            
            batch_preds = [bp if bp > 0 else None for bp in batch_preds]
