...
def download_yt_video(video: Union[str, YouTube], 
    ...
    try: 
        yt = video if isinstance(video, YouTube) else YouTube(video, 
                                                              use_oauth=True,
...
def youtube_playlist_dataset(playlist_url: str, 
    ...
    # extract the videos and their urls
    videos = p.videos
    video_urls = p.video_urls
    output_dir = dirf.process_save_path(output_dir)

    # define a text file to save the urls already downloaded
    # this way, we can avoid redownloading data 
    ...
    # a counter for the number of videos downloaded
    count = 0
    ...
    for index, (v, v_url) in enumerate(zip(videos, video_urls), start=1):
        ...
        if v_url not in existing_urls:
            ...
            # count the previously downloaded videos as well
            count += 1 
            continue

        output_dir = output_dir if save_in_one_folder else os.path.join(output_dir, f'video_{index}')

        # make sure to pass the 'Video' object to use authentication and avoid AgeRestrictedError
        video_path, fps = download_yt_video(v, 
        ...
        # stop the process when the number of downloaded videos reaches the limit (if explicitly set)
        if num_videos is not None and count >= num_videos:
            break   
